...
function tb_upload_action(el, url){
    var dom = jQuery(el)
    var ipt = dom.siblings('input[type=hidden]')
    var remdom = dom.siblings('.tbcus-remove')

    ipt.val( url )
    remdom.show()

    var exname = url.split('.').pop()
    url = url.replace('.'+exname, '-150x100.'+exname)
    dom.html('<img src="'+ url +'">')
}

function tb_upload_remove(el){
    var dom = jQuery(el)
    var ipt = dom.siblings('input[type=hidden]')
    var remdom = dom.siblings('.tbcus-remove')
